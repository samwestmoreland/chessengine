...
func computeKingAttacks(square int) uint64 {
	...
	if pieceBoard>>9 != 0 && pieceBoard>>9&notHFile == 1 {
		attackBoard |= (pieceBoard >> 9)
	}

	if (pieceBoard<<7) != 0 && (pieceBoard<<7)&notHFile == 1 {
		attackBoard |= (pieceBoard << 7)
	}

	if (pieceBoard>>7) != 0 && (pieceBoard>>7)&notAFile == 1 {
		attackBoard |= (pieceBoard >> 7)
	}
	...
	if (pieceBoard>>1) != 0 && pieceBoard>>1&notHFile == 1 {
		attackBoard |= (pieceBoard >> 1)
	}
